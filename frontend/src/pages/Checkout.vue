            ...
            <form id="checkoutForm" @submit="handleSubmit" novalidate autocomplete="off">
                ...
                <div class="form-group details-group">
                    ...
                    <div class="form-group">
                        <input type="text" name="coPhone" id="coPhone" placeholder="Telefonszám" class="form-control"
                            v-model="checkoutObj.phone" />
                        <p class="error-mess" v-if="errorObj.phoneErr.length > 0">{{ errorObj.phoneErr[0] }}</p>
                    </div>

                    <div class="form-group">
                        <input type="text" name="coAddress" id="coAddress" placeholder="Lakcím"
                ...
                <div class="form-group details-group">
                    ...
                    <div class="form-group">
                        <div class="form-group">
                            <input type="radio" name="payment" value="cash" id="paymentCash"
                                v-model="checkoutObj.paymentMethod" /><span>Készpénz</span>
                            <input type="radio" name="payment" value="card" id="paymentCard"
                                v-model="checkoutObj.paymentMethod" /><span>Kártya (Visa)</span>
                        </div>
                    ...
                    <div v-if="checkoutObj.paymentMethod == 'card'">
                        <div class="form-group">
                            <input type="text" name="coCardNum" placeholder="Kártya száma" id="coCardNum"
                        ...
                        <div class="form-group">
                            <input v-upcase type="text" name="coCardName" placeholder="Név a kártyán"
                        ...
                        <div class="form-group">
                            <div class="form-control">
                                <span
                                    style="font-size: 1.6rem; position: absolute; margin-left: -5px; margin-top: -11px;">Lejárat:
                                </span>
                                <input
                ...
                <div v-if="user" class="form-group">
                    <input type="submit" value="Elfogadás és fizetés" class="btn"
    ...
    methods: {
        ...
        checkForm: function () {
            ...
            if (!this.checkoutObj.phone) {
                ...
                if (!this.checkoutObj.phone.startsWith('+36') && !this.checkoutObj.phone.startsWith('06')) {
                    this.errorObj.phoneErr.push('Telefonszám csak +36-al vagy 06-al kezdődhet');
                }

                if (this.checkoutObj.phone.length != 11 && this.checkoutObj.phone.length != 12) {
                    this.errorObj.phoneErr.push('A telefonszám hosszúsága nem megfelelő');
                }

                if (!/[0-9]{11}/.test(this.checkoutObj.phone) && !/[+0-9]{12}/.test(this.checkoutObj.phone)) {
            ...
            if (!this.checkoutObj.address) {
                this.errorObj.addressErr.push('Lakcím bevitele kötelező');
            }

            // Card validate
            if (!this.checkoutObj.paymentMethod) {
                this.errorObj.payErr.push('Fizetési mód kiválasztása kötelező');
            }
            else if (this.checkoutObj.paymentMethod == "card") {
                if (!this.cardObj.number) {
                    this.errorObj.numErr.push('Kártyaszám bevitele kötelező');
                }
                else {
                    if (!this.cardObj.number.startsWith('4')) {
                        this.errorObj.numErr.push('Visa kártyák csak 4-essel kezdődhetnek');
                    }

                    if (this.cardObj.number.length != 16) {
                        this.errorObj.numErr.push('Visa kártya csak 16 számot tartalmazhat');
                    }

                    if (!/[0-9]{16}/.test(this.cardObj.number)) {
                        this.errorObj.numErr.push('Visa kártyában csak számok lehetnek');
                    }
                }
                ...
                if (!this.cardObj.expiryDate) {
                    this.errorObj.exDateErr.push('Lejárati dátum bevitele kötelező');
                }


                if (!this.cardObj.cvv) {
                    this.errorObj.cvvErr.push('CCV kód bevitele kötelező');
                }
                else {
                    if (this.cardObj.cvv.length != 3) {
                        this.errorObj.cvvErr.push('CCV kód csak 3 szám lehet');
                    }

                    if (!/[0-9]{3}/.test(this.cardObj.cvv)) {
                        this.errorObj.cvvErr.push('CCV kód csak szám lehet');
                    }
                }
